...
tensorboard_folder = "cifar10dense"
...
x = tf.layers.dense(inputs=x, units=1024, activation=tf.nn.elu)
x = tf.layers.dropout(x, rate=drop_rate, training = tf.not_equal(drop_rate, 1))
x = tf.layers.batch_normalization(x)
x = tf.layers.dense(inputs=x, units=768, activation=tf.nn.elu)
x = tf.layers.dropout(x, rate=drop_rate, training = tf.not_equal(drop_rate, 1))
x = tf.layers.batch_normalization(x)
x = tf.layers.dense(inputs=x, units=512, activation=tf.nn.elu)
x = tf.layers.dropout(x, rate=drop_rate, training = tf.not_equal(drop_rate, 1))
x = tf.layers.batch_normalization(x)
x = tf.layers.dense(inputs=x, units=512, activation=tf.nn.elu)
x = tf.layers.dropout(x, rate=drop_rate, training = tf.not_equal(drop_rate, 1))
...
out = tf.layers.dense(inputs=x, units=10, activation=tf.nn.elu)
...
for epoch in range(epochs):
    ...
    if include_tensorboard:
        ...
        train_loss, train_acc, summary = sess.run(
            ...
            {x_: train_x, y: train_y, drop_rate: 1}
        )
        writer.add_summary(summary, epoch)
    else:
        loss, acc = sess.run(
